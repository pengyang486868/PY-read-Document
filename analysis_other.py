...
def on_loop(project_id):
    ...
    analyzed = ['doc', 'docx', 'ppt', 'pptx', 'pdf', 'dwg']
    docdata = get_new_doc_task_db(project_id, filetype=None, notin=analyzed)
    if len(docdata) == 0:
        return
    ...
    # other_analysis = docdata['fileType'].map(lambda x: x not in analyzed)
    # docdata = docdata[other_analysis]
